if GetLocale() == "zhTW" then
  ...
  core.L = {
    ...
    MINUTES = "分鐘",
    MISCSETTINGS = "雜項設置",
    CulteDKPSCALESIZE = "整體式DKP秤尺寸",
    CulteDKPSCALESIZETTDESC = "CulteDKP窗口的比例。單擊\"保存設置\"以將大小更改為設置值。",
    CulteDKPSCALESIZETTWARN = "如果使用另一個修改UI比例的插件（例如TukUI，ElvUI等），則保存後可能需要/ reload。",
    MORE = "更多",
    ...
    確實要這樣做嗎？]=],
    OUTOFDATE = "過時的",
    OUTOFDATEANNOUNCE = "您的CulteDKP版本已過時。請更新Curse / Twitch或WoWInterface，以確保沒有兼容性問題。",
    PAUSERAID = "暫停RAID",
    ...
    TOTALCulteDKPUSERS = "CulteDKP用戶總數",
    TOWITHDRAWBID = "撤回出價。",
    TRINKET = "飾品",
    ...
    強烈建議您僅在希望限制哪些警官需要權限的情況下才使用此白名單設置。如果您希望所有人員都具有權限，請完全忽略此設置功能。請謹慎使用。 （如果應用這些設置會導致問題，請使用文本編輯器打開 \WTF\Accounts\ACCOUNT_NAME\SavedVariables\CulteDKP.lua文件，然後刪除底部附近的CulteDKP_Whitelist表。）]=],
    WIPETABLES = "擦桌子",
