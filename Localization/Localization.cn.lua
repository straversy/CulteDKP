if GetLocale() == "zhCN" then
  ...
  BIDSPECCHANGED = "Spec change was Accepted.",
    ...
    MELEEDPS = "近战DPS",
    MIGRATE = "迁移",
    MIGRATEINST1 = [=[|cffff0000IMPORTANT!!|r：在继续之前备份您保存的变量文件。 2.0版附带了一个全新的索引广播系统。这需要使用新值来迁移当前表。此迁移是一次性的过程，只能由一名人员执行。选择具有最准确数据的人员进行迁移。所有其他人员将被阻止，并将删除其数据，以接受来自所选人员的新数据广播。这是为了防止重复数据。一旦所有人员都收到了新数据，您就可以从行会负责人公开说明中删除{CulteDKP=OFFICER_NAME}标签。至关重要的是，只有一名管理人员执行迁移，否则您将有重复的数据并且您的表将被损坏（解决此问题的唯一方法是让所有损坏了表的人员注销并同时删除其保存的变量文件，否则登录后，您将再次收到损坏的数据）。此外，所有成员都必须使用2.0或更高版本才能兼容。旧版本将无法使用。

    可以使用“/dkp migration”重新打开此窗口。]=],
    MIGRATEINST2 = "％s 已经进行了迁移。请立即备份您保存的变量文件（WTF\\Account\\ACCOUNT_NAME\\SavedVariables\\CulteDKP.lua），然后单击下面的删除表按钮。如果 ％s 在线，他们将在您重新加载后自动向您推送同步。如果不是，将在他们登录时将其推送，或者您可以请求他们推送同步（这是通过单击主GUI左下角的表格状态按钮来完成的）。如果您的公会的迁移过程中发生任何错误，请从公会负责人的公开说明中删除迁移人员标签，并让到目前为止参与的所有人员删除已损坏的已保存变量文件，并将其还原到已备份的文件中。每个人都必须同时执行此操作，并且在完成此操作之前，请勿登录（启用插件）。拥有迁移表的任何在线用户都将在您登录后自动将数据推送给您。",
    MIN = "敏",
    ...
    MINUTES = "分钟",
    MISCSETTINGS = "杂项设置",
    CulteDKPSCALESIZE = "整体式DKP秤尺寸",
    CulteDKPSCALESIZETTDESC = "CulteDKP窗口的比例。单击\"保存设置\"以将大小更改为设置值。",
    CulteDKPSCALESIZETTWARN = "如果使用另一个修改UI比例的插件（例如TukUI，ElvUI等），则保存后可能需要/ reload。",
    MORE = "更多",
    ...
    您确定要这样做吗]=],
    OUTOFDATE = "过时的",
    OUTOFDATEANNOUNCE = "您的CulteDKP版本已过时。请更新Curse / Twitch或WoWInterface，以确保没有兼容性问题。",
    PAUSERAID = "暂停突",
    ...
    TOTALCulteDKPUSERS = "CulteDKP用户总数",
    TOWITHDRAWBID = "撤回出价。",
    TRINKET = "饰品",
    ...
    强烈建议您仅在希望限制哪些警官需要权限的情况下才使用此白名单设置。如果您希望所有人员都具有权限，请完全忽略此设置功能。请谨慎使用。 （如果应用这些设置会导致问题，请使用文本编辑器打开\WTF\Accounts\ACCOUNT_NAME\SavedVariables\CulteDKP.lua文件，然后删除底部附近的CulteDKP_Whitelist表。）]=],
    WIPETABLES = "擦桌子",
