...
function CulteDKP:VerifyMinBidItemTable(dbTable)
    local newTable = {};

    if dbTable.dbinfo.name ~= "CulteDKP_MinBids" then
        return dbTable;
    end

    CulteDKP:Print("Beginning MinBid Table Verification...");
        ...
        if realmName == "__default" or realmName == "dbinfo" then
            ...
            for guildName, guild in pairs(realm) do
                ...
                for teamId, oldTeamItems in pairs(guild) do
                    ...
                    CulteDKP:Print("Updating "..guildName.." Team "..teamId.." on "..realmName);
                    local numItems = 0;
    ...
    CulteDKP:Print("Finished MinBid Table Verification!");
    return newTable
end

function CulteDKP:RefactorMinBidItemTable(dbTable)
    local newTable = {};

    if dbTable.dbinfo.name ~= "CulteDKP_MinBids" then
        return dbTable;
    end
    CulteDKP:Print("Beginning MinBid Table Upgrade...");
    for realmName, realm in pairs(dbTable) do
        if realmName == "__default" or realmName == "dbinfo" then
            -- System Objects
            newTable[realmName] = realm;
    ...
    CulteDKP:Print("Finished MinBid Table Upgrade!");
    return newTable
end
