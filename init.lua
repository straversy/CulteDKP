...
CulteDKP.Commands = {
  ["config"] = function()
    if core.Initialized then
      local pass, err = pcall(CulteDKP.Toggle)
      if not pass then
        CulteDKP:Print(err)
      ...
      CulteDKP:Print("CulteDKP has not completed initialization.")
    end
  end,
  ["reset"] = CulteDKP.ResetPosition,
  ["bid"] = function(...)
    if core.Initialized then
      local item = strjoin(" ", ...)
      ...
      if core.IsOfficer then  
		if ... == nil then
			if core.BidAuctioneer then
				CulteDKP:ToggleBidWindow()
			else
				CulteDKP:BidInterface_Toggle()
			end
        else
          local itemName,_,_,_,_,_,_,_,_,itemIcon = GetItemInfo(item)
          CulteDKP:Print("Opening Bid Window for: ".. item)
          CulteDKP:ToggleBidWindow(item, itemIcon, itemName)
        end
      end
      CulteDKP:BidInterface_Toggle()
    else
  ...
  ["repairtables"] = function(...)       -- test new features
    ...
    if core.IsOfficer then
      if cmd == "true" then
        CulteDKP:RepairTables(cmd)
      else
        CulteDKP:RepairTables()
      end
    end
  end,
  ["award"] = function (name, ...)
    if core.IsOfficer and core.Initialized then
	  ...
	  if not search then
		cost = CulteDKP:GetMinBid(item)
	  else
		cost = search.minbid;
	  end

	  CulteDKP:AwardConfirm(nil, cost, core.DB.bossargs.LastKilledBoss, core.DB.bossargs.CurrentRaidZone, item)
    else
  ...
  ["lockouts"] = function()
    ...
    lockouts:SetScript("OnEvent", CulteDKP_OnEvent);
    RequestRaidInfo()
  end,
  ["timer"] = function(time, ...)
    if time == nil then
      CulteDKP:BroadcastTimer(1, "...")
    else
      local title = strjoin(" ", ...)
      CulteDKP:BroadcastTimer(tonumber(time), title)
    end
  end,
  ["export"] = function(time, ...)
    CulteDKP:ToggleExportWindow()
  end,
  ["modes"] = function()
    if core.Initialized then
      CulteDKP:CheckOfficer()
  ...
  ["zone"] = function()
    CulteDKP:Print("CulteDKP zone");
    local zoneIndex = GetZoneText();
    CulteDKP:Print(zoneIndex.." : "..core.DB.bossargs.CurrentRaidZone.." : "..core.DB.bossargs.LastKilledBoss);
  end,
  ["help"] = function()
    CulteDKP:Print(" ");
    CulteDKP:Print(L["SLASHCOMMANDLIST"]..":")
    CulteDKP:Print("|cff00cc66/dkp|r - "..L["DKPLAUNCH"]);
    CulteDKP:Print("|cff00cc66/dkp ?|r - "..L["HELPINFO"]);
    CulteDKP:Print("|cff00cc66/dkp reset|r - "..L["DKPRESETPOS"]);
    CulteDKP:Print("|cff00cc66/dkp lockouts|r - "..L["DKPLOCKOUT"]);
    CulteDKP:Print("|cff00cc66/dkp timer|r - "..L["CREATERAIDTIMER"]);
    CulteDKP:Print("|cff00cc66/dkp bid|r - "..L["OPENBIDWINDOWHELP"]);
    CulteDKP:Print("|cff00cc66/dkp bid [itemlink]|r - "..L["OPENAUCWINHELP"]);
    CulteDKP:Print("|cff00cc66/dkp award [item link]|r - "..L["DKPAWARDHELP"]);
    CulteDKP:Print("|cff00cc66/dkp modes|r - "..L["DKPMODESHELP"]);
    CulteDKP:Print("|cff00cc66/dkp export|r - "..L["DKPEXPORTHELP"]);
    CulteDKP:Print(" ");
    CulteDKP:Print(L["WHISPERCMDSHELP"]);
    CulteDKP:Print("|cff00cc66!bid (or !bid <"..L["VALUE"]..">)|r - "..L["BIDHELP"]);
    CulteDKP:Print("|cff00cc66!dkp (or !dkp <"..L["PLAYERNAME"]..">)|r - "..L["DKPCMDHELP"]);
  end,
};

local function HandleSlashCommands(str)
  if (#str == 0) then
    CulteDKP.Commands.config();
    return;
  end  
...
ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER_INFORM", function(self, event, msg, ...)      -- suppresses outgoing whisper responses to limit spam
	...
	if core.DB.defaults.SuppressTells then
		...
		if strfind(msg, "CulteDKP: ") == 1 then
			return true
...
function CulteDKP_wait(delay, func, ...)
...
function CulteDKP:SendSeedData()

	local latestIndexForTeam = {}
	local _teams = CulteDKP:GetGuildTeamList();
	local _numberOfTeams = CulteDKP:tablelength(_teams);

	for i=1, _numberOfTeams do

		latestIndexForTeam[tostring(_teams[i][1])] = {}

		if 	#CulteDKP:GetTable(CulteDKP_DKPHistory, true, tostring(_teams[i][1])) > 0 and strfind(CulteDKP:GetTable(CulteDKP_DKPHistory, true, tostring(_teams[i][1]))[1].index, "-") then
			local off1,date1 = strsplit("-", CulteDKP:GetTable(CulteDKP_DKPHistory, true, tostring(_teams[i][1]))[1].index)
			if CulteDKP:ValidateSender(off1) then
				latestIndexForTeam[tostring(_teams[i][1])]["DKPHistory"] = CulteDKP:GetTable(CulteDKP_DKPHistory, true, tostring(_teams[i][1]))[1].index
			else
		...
		if #CulteDKP:GetTable(CulteDKP_Loot, true, tostring(_teams[i][1])) > 0 and strfind(CulteDKP:GetTable(CulteDKP_Loot, true, tostring(_teams[i][1]))[1].index, "-") then
			local off2,date2 = strsplit("-", CulteDKP:GetTable(CulteDKP_Loot, true, tostring(_teams[i][1]))[1].index)
			if CulteDKP:ValidateSender(off2) then
				latestIndexForTeam[tostring(_teams[i][1])]["Loot"] = CulteDKP:GetTable(CulteDKP_Loot, true, tostring(_teams[i][1]))[1].index
			else
	...
	CulteDKP.Sync:SendData("CDKPSeed", latestIndexForTeam) -- requests role and spec data and sends current seeds (index of newest DKP and Loot entries)

end

function CulteDKP_OnEvent(self, event, arg1, ...)

	if self:IsForbidden() then return; end;

	if event == "ADDON_LOADED" then
		if (arg1 ~= "CulteDKP") then return end
		core.IsOfficer = nil
		core.Initialized = false
		CulteDKP_wait(2, DoInit, event, arg1);
		---DoInit(event,arg1);
	...
	if core.DB == nil then
		...
		C_Timer.After(eventDelay[event], function () CulteDKP_OnEvent(self, event, arg1); end);
		return;
	end
	...
	if event == "BOSS_KILL" then
		...
		if core.IsOfficer and IsInRaid() then
			local boss_name = ...;
			if CulteDKP:Table_Search(core.EncounterList, arg1) then
				CulteDKP.ConfigTab2.BossKilledDropdown:SetValue(arg1)

				if core.DB.modes.StandbyOptIn and core.RaidInProgress then
					CulteDKP_Standby_Announce(boss_name)
				end
				...
				if core.DB.modes.AutoAward and core.RaidInProgress then
					if not core.DB.modes.StandbyOptIn and core.DB.DKPBonus.IncStandby then
						CulteDKP:AutoAward(3, core.DB.DKPBonus.BossKillBonus, core.DB.bossargs.CurrentRaidZone..": "..core.DB.bossargs.LastKilledBoss)
					else
						CulteDKP:AutoAward(1, core.DB.DKPBonus.BossKillBonus, core.DB.bossargs.CurrentRaidZone..": "..core.DB.bossargs.LastKilledBoss)
					end
				end
			else
				CulteDKP:Print("Event ID: "..arg1.." - > "..boss_name.." Killed. Please report this Event ID at https://www.curseforge.com/wow/addons/CulteDKP/issues to update raid event handlers.")
			end
	...
	elseif event == "ENCOUNTER_START" then
		if core.DB.defaults.AutoLog and IsInRaid() then
			if not LoggingCombat() then
				LoggingCombat(1)
				CulteDKP:Print(L["NOWLOGGINGCOMBAT"])
			end
		end
	elseif event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then   		-- logs 15 recent zones entered while in a raid party
		if IsInRaid() and core.Initialized then 					-- only processes combat log events if in raid
			CulteDKP:CheckOfficer()
			if core.IsOfficer then
				if not CulteDKP:Table_Search(core.DB.bossargs.RecentZones, GetRealZoneText()) then 	-- only adds it if it doesn't already exist in the table
			...
			if core.DB.defaults.AutoLog and CulteDKP:Table_Search(core.ZoneList, GetRealZoneText()) then
				if not LoggingCombat() then
					LoggingCombat(1)
					CulteDKP:Print(L["NOWLOGGINGCOMBAT"])
				end
	...
	elseif event == "UPDATE_INSTANCE_INFO" then
		...
		for k,v in pairs(core.DB.Lockouts) do
			...
			if k ~= "Three" then 	-- remove when three day raid lockouts are added
				CulteDKP:Print(raidString.." "..L["RESETSIN"]..days..hours..mins.." ("..date("%A @ %H:%M:%S%p", v)..")")
			end
		end
	...
	elseif event == "GROUP_ROSTER_UPDATE" then 			--updates raid listing if window is open
		if CulteDKP.UIConfig and core.CulteDKPUI:IsShown() then
	...
	elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then 		-- logs last 15 NPCs killed while in raid
		if IsInRaid() then 					-- only processes combat log events if in raid
			...
			if arg1 == "UNIT_DIED" and not strfind(arg2, "Player") and not strfind(arg2, "Pet-") then
				CulteDKP:CheckOfficer()
				if core.IsOfficer then
					if not CulteDKP:Table_Search(core.DB.bossargs.LastKilledNPC, arg3) then 	-- only adds it if it doesn't already exist in the table
		...
		if IsInRaid() then 					-- only processes combat log events if in raid
			...
			if arg1 == "UNIT_DIED" then
				...
				if core.IsOfficer == true then
					if CulteDKP:TableStrFind(core.BossList, arg2) then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue(arg2)
					elseif arg2 == "Flamewalker Elite" or arg2 == "Flamewalker Healer" then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue("Majordomo Executus")
					elseif arg2 == "Emperor Vek'lor" or arg2 == "Emperor Vek'nilash" then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue("Twin Emperors")
					elseif arg2 == "Princess Yauj" or arg2 == "Vem" or arg2 == "Lord Kri" then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue("Bug Family")
					elseif arg2 == "Highlord Mograine" or arg2 == "Thane Korth'azz" or arg2 == "Sir Zeliek" or arg2 == "Lady Blaumeux" then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue("The Four Horsemen")
					elseif arg2 == "Gri'lek" or arg2 == "Hazza'rah" or arg2 == "Renataki" or arg2 == "Wushoolay" then
						CulteDKP.ConfigTab2.BossKilledDropdown:SetValue("Edge of Madness")
					end
				end
			end
		end--]]
	elseif event == "LOOT_OPENED" then
		...
		if core.IsOfficer then
			...
			if IsInRaid() then
				...
				lootTable.boss=name
				CulteDKP.Sync:SendData("CDKPBossLoot", lootTable)
...
function CulteDKP:OnInitialize(event, name)		-- This is the FIRST function to run on load triggered registered events at bottom of file
	if (name ~= "CulteDKP") then return end 
	...
	----------------------------------
	SLASH_CulteDKP1 = "/dkp";
	SLASH_CulteDKP2 = "/CulteDKP";
	SlashCmdList.CulteDKP = HandleSlashCommands;
	...
	if(event == "ADDON_LOADED") then
		C_Timer.After(5, function ()
			core.CulteDKPUI = CulteDKP.UIConfig or CulteDKP:CreateMenu();		-- creates main menu after 5 seconds (trying to initialize after raid frames are loaded)
				...
				if core.Initialized and core.IsOfficer then
					...
					if GameTooltip:GetItem() then
						...
						if (key == "LALT" or key == "LSHIFT") and IsShiftKeyDown() and IsAltKeyDown() then
							...
							CulteDKP:ToggleBidWindow(itemLink, itemIcon, itemName);
						end

						if (key == "LCTRL" or key == "LSHIFT") and IsShiftKeyDown() and IsControlKeyDown() then
							-- TODO: Automate the Price a Bit More
							CulteDKP:AwardConfirm(nil, 0, core.DB.bossargs.LastKilledBoss, core.DB.bossargs.CurrentRaidZone, link);
						end
		...
		-- Verify DB Schemas
		------------------------------------------------\
		if not CulteDKP:VerifyDBSchema(CulteDKP_DB) then CulteDKP_DB = CulteDKP:UpgradeDBSchema(CulteDKP_DB, CulteDKP_DB, false, "CulteDKP_DB") end;
		
		-- Verify that the DB table has been initialized.
		CulteDKP:SetTable(CulteDKP_DB, false, CulteDKP:InitializeCulteDKPDB(CulteDKP:GetTable(CulteDKP_DB)))
		core.DB = CulteDKP:GetTable(CulteDKP_DB); --Player specific DB

		
		if not CulteDKP:VerifyDBSchema(CulteDKP_DKPTable) then CulteDKP_DKPTable = CulteDKP:UpgradeDBSchema(CulteDKP_DKPTable, CulteDKP_DKPTable, true, "CulteDKP_DKPTable") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_Loot) then CulteDKP_Loot = CulteDKP:UpgradeDBSchema(CulteDKP_Loot, CulteDKP_Loot, true, "CulteDKP_Loot") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_DKPHistory) then CulteDKP_DKPHistory = CulteDKP:UpgradeDBSchema(CulteDKP_DKPHistory, CulteDKP_DKPHistory, true, "CulteDKP_DKPHistory") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_MinBids) then CulteDKP_MinBids = CulteDKP:UpgradeDBSchema(CulteDKP_MinBids, CulteDKP_MinBids, true, "CulteDKP_MinBids") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_MaxBids) then CulteDKP_MaxBids = CulteDKP:UpgradeDBSchema(CulteDKP_MaxBids, CulteDKP_MaxBids, true, "CulteDKP_MaxBids") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_Whitelist) then CulteDKP_Whitelist = CulteDKP:UpgradeDBSchema(CulteDKP_Whitelist, CulteDKP_Whitelist, false, "CulteDKP_Whitelist") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_Standby) then CulteDKP_Standby = CulteDKP:UpgradeDBSchema(CulteDKP_Standby, CulteDKP_Standby, true, "CulteDKP_Standby") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_Archive) then CulteDKP_Archive = CulteDKP:UpgradeDBSchema(CulteDKP_Archive, CulteDKP_Archive, true, "CulteDKP_Archive") end;
		if not CulteDKP:VerifyDBSchema(CulteDKP_Profiles) then CulteDKP_Profiles = CulteDKP:UpgradeDBSchema(CulteDKP_Profiles, CulteDKP_Profiles, true, "CulteDKP_Profiles") end;


		------------------------------------
	    --	Import SavedVariables
	    ------------------------------------
		core.WorkingTable 		= CulteDKP:GetTable(CulteDKP_DKPTable, true); -- imports full DKP table to WorkingTable for list manipulation
		core.PriceTable			= CulteDKP:FormatPriceTable();

		for i=1, #core.WorkingTable do
			local CurPlayer = core.WorkingTable[i].player;
			local search = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_DKPTable, true), CurPlayer);

			if search then
				--Set Version Info on Legacy Record First
				local profile = CulteDKP:GetTable(CulteDKP_Profiles, true)[CurPlayer] or CulteDKP:GetDefaultEntity();
				core.WorkingTable[i].version = profile.version;
			end
		end


		if not CulteDKP:GetTable(CulteDKP_DKPHistory, true).seed then CulteDKP:GetTable(CulteDKP_DKPHistory, true).seed = 0 end
		if not CulteDKP:GetTable(CulteDKP_Loot, true).seed then CulteDKP:GetTable(CulteDKP_Loot, true).seed = 0 end
		if CulteDKP:GetTable(CulteDKP_DKPTable, true).seed then CulteDKP:GetTable(CulteDKP_DKPTable, true).seed = nil end
		...
		CulteDKP:StartBidTimer("seconds", nil)						-- initiates timer frame for use

		if CulteDKP.BidTimer then CulteDKP.BidTimer:SetScript("OnUpdate", nil) end

		if #CulteDKP:GetTable(CulteDKP_Loot, true) > core.DB.defaults.HistoryLimit then
			CulteDKP:PurgeLootHistory()									-- purges Loot History entries that exceed the "HistoryLimit" option variable (oldest entries) and populates CulteDKP_Archive with deleted values
		end
		if #CulteDKP:GetTable(CulteDKP_DKPHistory, true) > core.DB.defaults.DKPHistoryLimit then
			CulteDKP:PurgeDKPHistory()									-- purges DKP History entries that exceed the "DKPHistoryLimit" option variable (oldest entries) and populates CulteDKP_Archive with deleted values
		end
		core.DB.pendingLoot = {}
	end
end

function CulteDKP:GetTable(dbTable, hasTeams, teamIndex)
	...
	if IsInGuild() then
		...
		dbTable = CulteDKP:InitializeGuild(dbTable,realmName,guildName);
...
function CulteDKP:SetTable(dbTable, hasTeams, value, teamIndex)
	hasTeams = hasTeams or false;
	local _teamIndex;
...
function CulteDKP:VerifyDBSchema(dbTable)
	local verified = false;
	--Check to see if the schema node exists. If not, this is 2.1.2 database.
	local retOK, hasInfo = pcall(CulteDKP_tableHasKey,dbTable,"dbinfo");
...
function CulteDKP:UpgradeDBSchema(newDbTable, oldDbTable, hasTeams, tableName)
	-- Initialize the Database for a Pre-2.2 Database
	local retOK, hasInfo = pcall(CulteDKP_tableHasKey,oldDbTable,"dbinfo");
	if (not retOK) or (retOK and not hasInfo) then
		newDbTable = CulteDKP:InitPlayerTable(oldDbTable, hasTeams, tableName)
	end
	...
	if newDbTable.dbinfo.build < 20205 and newDbTable.dbinfo.priorbuild ~= core.BuildNumber then
		if newDbTable.dbinfo.name == "CulteDKP_DB" then

			local defaultTable = {}
			defaultTable = CulteDKP:InitializeCulteDKPDB(CulteDKP:GetTable(defaultTable))
	...
	if newDbTable.dbinfo.build < 30200 and newDbTable.dbinfo.priorbuild ~= core.BuildNumber and newDbTable.dbinfo.priorbuild ~= 0 then
		if newDbTable.dbinfo.name == "CulteDKP_MinBids" then
			newDbTable = CulteDKP:RefactorMinBidItemTable(newDbTable);
		end
	end
	...
	if newDbTable.dbinfo.build < 30202 and newDbTable.dbinfo.priorbuild ~= core.BuildNumber and newDbTable.dbinfo.priorbuild ~= 0 then
		if newDbTable.dbinfo.name == "CulteDKP_MinBids" then
			newDbTable = CulteDKP:VerifyMinBidItemTable(newDbTable);
		end
	end
...
function CulteDKP_tableHasKey(table,key)
	return table[key] ~= nil;
end

function CulteDKP:InitializeCulteDKPDB(dbTable)
	if dbTable == nil then
		dbTable = {}
	end
	...
	if not dbTable.defaults or not dbTable.defaults.HistoryLimit then
		dbTable.defaults = {
			HistoryLimit = 2500, DKPHistoryLimit = 2500, BidTimerSize = 1.0, CulteDKPScaleSize = 1.0, SuppressNotifications = false, TooltipHistoryCount = 15, SuppressTells = true,
		}
	end
	...
	if IsInGuild() then
		if not dbTable.teams["0"] then 
			dbTable.teams["0"] = {name=CulteDKP:GetGuildName()}
...
function CulteDKP:InitPlayerTable(globalTable, hasTeams, tableName)
	local playerTable = {};
	playerTable = CulteDKP:InitDbSchema(playerTable, tableName);
	if IsInGuild() then
		...
		playerTable = CulteDKP:InitializeGuild(playerTable,realmName,guildName);
...
function CulteDKP:InitializeGuild(dataTable, realmName, guildName)
	local retOK1, hasInfo1 = pcall(CulteDKP_tableHasKey,dataTable,realmName);
	...
	if guildName ~= nil then
		local retOK2, hasInfo2 = pcall(CulteDKP_tableHasKey,dataTable[realmName],guildName);
...
function CulteDKP:InitDbSchema(dbTable, tableName)
	dbTable["dbinfo"] = {};
...
function CulteDKP:MonolithMigration()
	...
	if not finished then
		CulteDKP:Print("MonolithDKP has not finished loading - please report to Discord!")
		return false -- Should not happen?! Maybe race condition ...
	end

	-- MonolithDKP is up & running - CulteDKP will not initialise
	CulteDKP:Print("Legacy MonolithDKP addon detected - please disable it to continue with CulteDKP!")
	local activeCommunity = self:MonolithMigrationDbEntries("CulteDKP") -- check if CulteDKP already has table entries
	local activeCommunitySchema = CulteDKP_DB ~= nil and CulteDKP:GetTable(CulteDKP_DB, false)["teams"] ~= nil -- check if CulteDKP finished schema migration
	...
	if not activeCommunity and (activeMonolith21x or activeMonolith21x) then
		-- CulteDKP is fresh and there are MonolithDKP 2.1.x or 2.2.x tables available
		self:MonolithMigrationLegacyDetected(function() self:MonolithMigrationProcess(false) end)
	elseif activeCommunity and activeCommunitySchema and activeMonolith21x and IsInGuild() and CulteDKP:ValidateSender(UnitName("player")) then
		-- CulteDKP already has data we can import MonolithDKP 2.1.x data as a new team
		self:MonolithMigrationAsNewTeam(function() self:MonolithMigrationProcess(true) end)
	else
 		-- CulteDKP already has table entries and there is no legacy data to add as an additional team
		self:MonolithMigrationGenericPopup(L["MIGRATIONUNAVAILABLE"])
	end

	return true -- don't initialise CulteDKP
end

function CulteDKP:MonolithMigrationProcess(asNewTeam)
	asNewTeam = asNewTeam or false;

 	-- add a new team?
	local teamIndex
	if asNewTeam then
		teamIndex = CulteDKP:AddNewTeamToGuild()
	end
	...
	copyTableRecursive = function(obj)
 	...
 	-- rename MonDKPScaleSize property to CulteDKPScaleSize for each guild / team
	local migrateDefaultsRecursive
	migrateDefaultsRecursive = function(table)
		for k, v in pairs(table) do
			if k == "defaults" then
				if v.MonDKPScaleSize ~= nil then
					v.CulteDKPScaleSize = v.MonDKPScaleSize
	...
	if asNewTeam then
	 ...
	 -- CulteDKP:SetTable(CulteDKP_DB,         true, tempDB,            teamIndex)

	 -- CulteDKP:GetTable(CulteDKP_DKPTable,   false)[teamIndex] = MonDKP_DKPTable
		CulteDKP:SetTable(CulteDKP_DKPTable,   true, MonDKP_DKPTable,   teamIndex)
		CulteDKP:SetTable(CulteDKP_Loot,       true, MonDKP_Loot,       teamIndex)
		CulteDKP:SetTable(CulteDKP_DKPHistory, true, MonDKP_DKPHistory, teamIndex)
		CulteDKP:SetTable(CulteDKP_MinBids,    true, MonDKP_MinBids,    teamIndex)
		CulteDKP:SetTable(CulteDKP_MaxBids,    true, MonDKP_MaxBids,    teamIndex)
		CulteDKP:SetTable(CulteDKP_Whitelist,  true, MonDKP_Whitelist,  teamIndex)
		CulteDKP:SetTable(CulteDKP_Standby,    true, MonDKP_Standby,    teamIndex)
		CulteDKP:SetTable(CulteDKP_Archive,    true, MonDKP_Archive,    teamIndex)
	else
		local tempDB = copyTableRecursive(MonDKP_DB)
		migrateDefaultsRecursive(tempDB)
		CulteDKP_DB         = tempDB

		CulteDKP_DKPTable   = MonDKP_DKPTable
		CulteDKP_Loot       = MonDKP_Loot
		CulteDKP_DKPHistory = MonDKP_DKPHistory
		CulteDKP_MinBids    = MonDKP_MinBids
		CulteDKP_MaxBids    = MonDKP_MaxBids
		CulteDKP_Whitelist  = MonDKP_Whitelist
		CulteDKP_Standby    = MonDKP_Standby
		CulteDKP_Archive    = MonDKP_Archive
	end
...
function CulteDKP:MonolithMigrationLegacySeed()
...
function CulteDKP:MonolithMigrationDbBuild()
...
function CulteDKP:MonolithMigrationDbEntries(prefix)
	-- returns true if there are already CulteDKP entries
	...
	if prefix ~= nil and prefix == "MonDKP" then
		...
		return findEntryRecursive(CulteDKP_DKPHistory, "players")
			or findEntryRecursive(CulteDKP_DKPTable, "player")
			or findEntryRecursive(CulteDKP_Loot, "player")
	end
end
...
events:SetScript("OnEvent", CulteDKP_OnEvent); -- calls the above CulteDKP_OnEvent function to determine what to do with the event
