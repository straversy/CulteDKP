...
function CulteDKP:ToggleDKPModesWindow()
	if core.IsOfficer == true then
		if not core.ModesWindow then
			core.ModesWindow =  CulteDKP:DKPModesFrame_Create();

			-- Populate Tabs
			CulteDKP:DKPModes_Main()
			CulteDKP:DKPModes_Misc()
		end
...
function CulteDKP:DKPModesFrame_Create()

	local f;

	if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
		f = CreateFrame("Frame", "CulteDKP_DKPModesFrame", UIParent);
	else -- WOW_PROJECT_BURNING_CRUSADE_CLASSIC OR WOW_PROJECT_WRATH_CLASSIC
		f = CreateFrame("Frame", "CulteDKP_DKPModesFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil);
	end
...
-- TODO YOZO BACKGROUND
	f.BG:SetTexture("Interface\\AddOns\\CulteDKP\\Media\\Textures\\menu-bg");
--	f.BG:SetTexture("Interface\\AddOns\\CulteDKP\\Media\\Textures\\Culte-dkp_large");
	...
	f.DKPModesMain, f.DKPModesMisc = CulteDKP:SetTabs(f, 2, 475, 600, "Modes", "Misc");
	tinsert(UISpecialFrames, f:GetName()); -- Sets frame to close on "Escape"
