...
local function SetItemPrice(cost, loot)
	...
	elseif search then
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].minbid = CulteDKP_round(cost, core.DB.modes.rounding);
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].lastbid = CulteDKP_round(cost, core.DB.modes.rounding);
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].link = itemLink;
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].icon = itemIcon;
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].item = itemName;
		CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].itemID = itemID;
		if cost == 0 then
			CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].disenchants = 0;
		end
...
local function AwardItem(player, cost, boss, zone, loot, reassign)
	...
	if core.IsOfficer then
		if core.DB.modes.costvalue == "Percent" then
			local search = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_DKPTable, true), winner);

			if core.DB.modes.mode == "Roll Based Bidding" then
				if search then
					cost = CulteDKP:GetTable(CulteDKP_DKPTable, true)[search[1][1]].dkp * (cost / 100)
					cost = CulteDKP_round(cost, core.DB.modes.rounding);
				else
					print(L["ERROR"])
				end
			else
		...
		if reassign then
			search_reassign = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_Loot, true), reassign, "index")

			if search_reassign then
				local deleted = CopyTable(CulteDKP:GetTable(CulteDKP_Loot, true)[search_reassign[1][1]])
				local reimburse = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_DKPTable, true), deleted.player, "player")
				...
				CulteDKP:GetTable(CulteDKP_Loot, true)[search_reassign[1][1]].deletedby = newIndex
				CulteDKP:GetTable(CulteDKP_DKPTable, true)[reimburse[1][1]].dkp = CulteDKP:GetTable(CulteDKP_DKPTable, true)[reimburse[1][1]].dkp + deleted.cost
				CulteDKP:GetTable(CulteDKP_DKPTable, true)[reimburse[1][1]].lifetime_spent = CulteDKP:GetTable(CulteDKP_DKPTable, true)[reimburse[1][1]].lifetime_spent + deleted.cost
				table.insert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, deleted)
				CulteDKP.Sync:SendData("CDKPDelLoot", CulteDKP:GetTable(CulteDKP_Loot, true)[1])
			end
		end

		if core.DB.modes.StoreBids and not reassign then
			local Bids_Submitted = CulteDKP:BidsSubmitted_Get();
			...
			if Bids_Submitted[1] then
				if Bids_Submitted[1].bid then
					tinsert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, {player=winner, loot=loot, zone=curZone, date=curTime, boss=curBoss, cost=cost, index=newIndex, bids={ }})
					for k,v in pairs(BidsEntry) do
						table.insert(CulteDKP:GetTable(CulteDKP_Loot, true)[1].bids, {bidder=k, bid=v});
					end
				elseif Bids_Submitted[1].dkp then
					tinsert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, {player=winner, loot=loot, zone=curZone, date=curTime, boss=curBoss, cost=cost, index=newIndex, dkp={ }})
					for k,v in pairs(BidsEntry) do
						table.insert(CulteDKP:GetTable(CulteDKP_Loot, true)[1].dkp, {bidder=k, dkp=v});
					end
				elseif Bids_Submitted[1].roll then
					tinsert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, {player=winner, loot=loot, zone=curZone, date=curTime, boss=curBoss, cost=cost, index=newIndex, rolls={ }})
					for k,v in pairs(BidsEntry) do
						table.insert(CulteDKP:GetTable(CulteDKP_Loot, true)[1].rolls, {bidder=k, roll=v});
					end
				end
			else
				tinsert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, {player=winner, loot=loot, zone=curZone, date=curTime, boss=curBoss, cost=cost, index=newIndex})
			end
		else
			local newIndex = curOfficer.."-"..curTime
			tinsert(CulteDKP:GetTable(CulteDKP_Loot, true), 1, {player=winner, loot=loot, zone=curZone, date=curTime, boss=curBoss, cost=cost, index=newIndex})
			if reassign then
				local search = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_Loot, true), reassign, "index")

				if search and CulteDKP:GetTable(CulteDKP_Loot, true)[search[1][1]].player ~= winner then
					CulteDKP:GetTable(CulteDKP_Loot, true)[1].reassigned = true
				end
			end 
			if type(bids) == "table" then
				CulteDKP:GetTable(CulteDKP_Loot, true)[1].bids = bids
			end
		end
		CulteDKP:BidsSubmitted_Clear()
		CulteDKP.Sync:SendData("CDKPLootDist", CulteDKP:GetTable(CulteDKP_Loot, true)[1])
		CulteDKP:DKPTable_Set(winner, "dkp", CulteDKP_round(cost, core.DB.modes.rounding), true)
		...
		if core.BiddingWindow and core.BiddingWindow:IsShown() then  -- runs below if award is through bidding window (update minbids and zerosum bank)
			...
			local search = CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID]
			local minBidAmount = CulteDKP_round(CulteDKP:GetMinBid(loot), core.DB.modes.rounding);
			local lastBidAmount = CulteDKP_round(core.BiddingWindow.cost:GetNumber(), core.DB.modes.rounding);
			...
			if mode == "Static Item Values" or mode == "Roll Based Bidding" or (mode == "Zero Sum" and core.DB.modes.ZeroSumBidType == "Static") then
				if core.BiddingWindow.CustomMinBid:GetChecked() == false then
					minBidAmount = CulteDKP_round(minBidAmount, core.DB.modes.rounding);
				else
					if core.BiddingWindow.cost:GetNumber() ~= tonumber(minBidAmount) then
						minBidAmount = CulteDKP_round(core.BiddingWindow.cost:GetNumber(), core.DB.modes.rounding);
					end
				end
			else
				if core.BiddingWindow.CustomMinBid:GetChecked() == true then
					minBidAmount = CulteDKP_round(core.BiddingWindow.minBid:GetNumber(), core.DB.modes.rounding);
				end
			end
			...
			if not search then
				--not found
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID] = minBidEntry;
				if mode == "Static Item Values" or mode == "Roll Based Bidding" or (mode == "Zero Sum" and core.DB.modes.ZeroSumBidType == "Static") then
					core.BiddingWindow.CustomMinBid:SetShown(true);
					core.BiddingWindow.CustomMinBid:SetChecked(core.DB.defaults.CustomMinBid);
				end
				CulteDKP.Sync:SendData("CDKPSetPrice", minBidEntry);
			else
				--found
				if CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].minbid ~= minBidAmount then
					if mode == "Static Item Values" or mode == "Roll Based Bidding" or (mode == "Zero Sum" and core.DB.modes.ZeroSumBidType == "Static") then
						core.BiddingWindow.CustomMinBid:SetShown(true);
						core.BiddingWindow.CustomMinBid:SetChecked(core.DB.defaults.CustomMinBid);
					end
				...
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].minbid = minBidEntry.minbid;
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].lastbid = minBidEntry.lastbid;
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].link = minBidEntry.link;
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].icon = minBidEntry.icon;
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].item = minBidEntry.itemName;
				CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID].itemID = minBidEntry.itemID;

				CulteDKP.Sync:SendData("CDKPSetPrice", CulteDKP:GetTable(CulteDKP_MinBids, true)[itemID]);
			end
...
local function AwardConfirm_Create()
	...
	if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
		f = CreateFrame("Frame", "CulteDKP_AwardWindowConfirm", UIParent, "ShadowOverlaySmallTemplate");
	else -- WOW_PROJECT_BURNING_CRUSADE_CLASSIC OR WOW_PROJECT_WRATH_CLASSIC
		f = CreateFrame("Frame", "CulteDKP_AwardWindowConfirm", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil);
	end	
	...
	f.confirmHeader:SetFontObject("CulteDKPLargeRight");
	...
	----------------------------------
		...
		f.teamHeader:SetFontObject("CulteDKPLargeRight");
		f.teamHeader:SetScale(0.7)
		f.teamHeader:SetPoint("TOPLEFT", f, "TOPLEFT", 120, -60);
		f.teamHeader:SetText(L["TEAM"]..":")

		--f.team = CreateFrame("FRAME", "CulteDKPAwardConfirmPlayerDropDown", f, "CulteDKPUIDropDownMenuTemplate")
		f.team = LibDD:Create_UIDropDownMenu("CulteDKPAwardConfirmPlayerDropDown", f);
	...
	----------------------------------	
		...
		f.playerHeader:SetFontObject("CulteDKPLargeRight");
	...
	----------------------------------	
		...
		f.lootHeader:SetFontObject("CulteDKPLargeRight");
		...
		f.loot:SetFontObject("CulteDKPNormalLeft");
	...
	----------------------------------
		...
		f.costFooter:SetFontObject("CulteDKPNormalLeft");
	...
	----------------------------------
		...
		--f.bossDropDown = CreateFrame("FRAME", "CulteDKPAwardConfirmBossDropDown", f, "CulteDKPUIDropDownMenuTemplate")
		f.bossDropDown = LibDD:Create_UIDropDownMenu("CulteDKPAwardConfirmBossDropDown", f);
	...
	----------------------------------
		...
		f.zoneHeader:SetFontObject("CulteDKPLargeRight");
		f.zoneHeader:SetScale(0.7)
		f.zoneHeader:SetPoint("TOPRIGHT", f.bossHeader, "BOTTOMRIGHT", 0, -10);
		f.zoneHeader:SetText(L["ZONE"]..":")

		--f.zoneDropDown = CreateFrame("FRAME", "CulteDKPAwardConfirmBossDropDown", f, "CulteDKPUIDropDownMenuTemplate")
		f.zoneDropDown = LibDD:Create_UIDropDownMenu("CulteDKPAwardConfirmBossDropDown", f);
	...
	----------------------------------
		...
		f.yesButton = CulteDKP:CreateButton("BOTTOMLEFT", f, "BOTTOMLEFT", 20, 15, L["CONFIRM"]);
		f.setPriceButton = CulteDKP:CreateButton("BOTTOMLEFT", f, "BOTTOMLEFT", 150, 15, "Set Price");
		f.setPriceButton:SetShown(false);
		f.noButton = CulteDKP:CreateButton("BOTTOMRIGHT", f, "BOTTOMRIGHT", -20, 15, L["CANCEL"]);
...
function CulteDKP:AwardConfirm(player, cost, boss, zone, loot, reassign)
	...
	if player then
		search = CulteDKP:Table_Search(CulteDKP:GetTable(CulteDKP_DKPTable, true), player)
		class = CulteDKP:GetCColors(CulteDKP:GetTable(CulteDKP_DKPTable, true)[search[1][1]].class)
	end

	for i=1, #CulteDKP:GetTable(CulteDKP_DKPTable, true) do
		table.insert(PlayerList, CulteDKP:GetTable(CulteDKP_DKPTable, true)[i].player)
	end
	...
	-- Create and bind the initialization function to the dropdown menu
		...
		LibDD:UIDropDownMenu_SetText(core.AwardConfirm.team, CulteDKP:GetCurrentTeamName())
	...
	LibDD:UIDropDownMenu_Initialize(core.AwardConfirm.bossDropDown, function(self, level, menuList)                                   -- BOSS dropdown
		...
		for k,v in pairs(core.LastKilledNPC) do             -- eliminates duplicate zones
			if not CulteDKP:Table_Search(tempNPCs, v) then
	...
	LibDD:UIDropDownMenu_Initialize(core.AwardConfirm.zoneDropDown, function(self, level, menuList)                                   -- ZONE dropdown
		...
		for k,v in pairs(core.RecentZones) do             -- eliminates duplicate zones
			if not CulteDKP:Table_Search(tempZones, v) then
